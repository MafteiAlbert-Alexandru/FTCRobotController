 ...
 */
public class LocalTelemetry implements Telemetry, TelemetryInternal
{
    ...
    public LocalTelemetry()
    {
        ...
        file = new File(String.format("%s/FIRST/%s", Environment.getExternalStorageDirectory(), String.format("%s.log", date)));
    ...
    private void logTelemetryMessage(TelemetryMessage message)
    {
        try {
            ...
            byte isSorted = buffer.get();
            byte robotState = buffer.get();
            byte tagLength = buffer.get();

            for(int i=0;i<tagLength;i++) buffer.get();

            byte dataStringsLength=buffer.get();
            for(int i=0;i<dataStringsLength;i++)
            {
                byte dataStringLength = buffer.get();
                byte[] dataString = new byte[dataStringLength];
                for(int j=0;j<dataStringLength;j++)dataString[j]=buffer.get();
                byte dataValueLength = buffer.get();
                byte[] dataValue = new byte[dataValueLength];
                for(int j=0;j<dataValueLength;j++) dataValue[j]=buffer.get();
                writer.write(String.format("%d: %s %s\n", timestamp, new String(dataString, StandardCharsets.UTF_8),  new String(dataValue, StandardCharsets.UTF_8)));
            }
            byte dataNumbersLength=buffer.get();
            for(int i=0;i<dataNumbersLength;i++)
            {
                byte keyLength =buffer.get();
                byte[] key = new byte[keyLength];
                for(int j=0;j<keyLength;j++) key[j]=buffer.get();
                writer.write(String.format("%d: %s %f\n", timestamp, new String(key, StandardCharsets.UTF_8), buffer.getFloat()));
            }
    ...
    public LocalTelemetry(OpMode opMode)
    {
        this.opMode = opMode;
    ...
    protected boolean tryUpdate(UpdateReason updateReason)
    {
        synchronized (theLock)
        {
            ...
            if (intervalElapsed && wantToTransmit)
            {
                ...
                {
                    logTelemetryMessage(transmitter);
                }
